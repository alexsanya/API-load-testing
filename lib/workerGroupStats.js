(() => {
  ...
  module.exports = function (q, logUpdate) {
    ...
    class WorkersGroupStats {
      ...
      getAvgStatsForWorker(workerType) {
        ...
        for (const [workerId, workerStats] of this.overallStats[workerType]) {
          const timeDiff = toMilliseconds(process.hrtime(workerStats.timestamp));
          if (timeDiff > STATS_INFO_LIFETIME) {
            this.overallStats[workerType].delete(workerId);
          } else {
